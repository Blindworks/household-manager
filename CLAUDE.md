...
- **Spaziergänge-Dialog:** Klick auf die Hund-Kachel im Dashboard-Footer öffnet einen Dialog mit den Spaziergängen der letzten 7 Tage. Es gibt keinen Walks-Endpunkt in der Tractive-API (das App-Feature ist nicht reverse-engineert) — `GET /v1/tractive/pets/{trackerId}/walks?days` leitet sie stattdessen on-the-fly aus der Positionshistorie ab (`GET /tracker/{id}/positions`, wie die Geofences unverifiziert → defensives Parsen; **Abruf zwingend in 24-h-Häppchen** — die Cloud lehnt größere Fenster mit Code 7500 HISTORY „time frame is invalid" ab, real beobachtet bei 7 Tagen. Einzelne fehlgeschlagene Häppchen werden toleriert, weil im Basic-Abo die Historie nur 24 h zurückreicht — erst wenn alle scheitern, gibt es einen Fehler): zusammenhängende Zeiträume außerhalb des Home-Radius, Lücken bis einschließlich 10 min überbrückt (erst eine echt größere Lücke trennt — ein 10-Minuten-Meldeintervall darf keine Dauersplits erzeugen), Runden < 5 min verworfen (`TractiveWalkDetector`). Jede Abwesenheit zählt als „Spaziergang" (auch Autofahrten), und im Stromsparmodus können kurze Runden verschluckt werden — bewusste Eigenschaft der Datenlage. Ergebnis 5 min pro (Tracker, days) gecacht; ohne konfiguriertes Zuhause 400 mit klarer Meldung
