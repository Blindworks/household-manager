...
- Frontend: Admin-Seite „Anwesenheit" (`pages/admin-presence/`, Route `admin/presence`) mit Karenzzeit, Geräteliste (Personen-Dropdown aus `GET /v1/users`) und der Spalte „Zuletzt gesehen" aus `GET /status`; schlägt dieser Abruf fehl, bleibt die Spalte leer, ohne die Pflege zu blockieren. Anzeige seit 2026-08-26 in der Dashboard-**Kopfzeile** (`lumina__clock`, lumina-Kapselung, nicht mehr im Footer): ein Kreis mit Initialen und sichtbarem Namen je Person, Refresh 30 s, ein fehlgeschlagener Refresh behält den letzten Stand. `presenceRingClass`/`presenceLabel` zählen alle vier Zustände einzeln auf und haben **kein `default`** — mit `noImplicitReturns` wird eine künftige fünfte Ausprägung von `PresencePersonState` zum Compilerfehler statt still auf dem falschen Ring zu landen (Gegenstück zu `PresenceEvaluator.entityState`). Anders als die frühere Footer-Kachel hat der Ring nur **drei** visuelle Zustände: `unavailable` und `unknown` teilen sich denselben neutralen Ring (grau) statt eigener Symbole — eine bewusste, für dieses Wandtablet getroffene Entscheidung, dass beide NIE wie `off` (rot) aussehen dürfen. Ihr unterscheidender Text sitzt dadurch nur noch im `title`/`aria-label`-Tooltip, den das berührungslos laufende KIOSK-Wandtablet nicht anzeigen kann; `off` bekommt deshalb zusätzlich reduzierte Deckkraft als farbunabhängige zweite Ebene (Rot-Grün-Sehschwäche), und der Name steht sichtbar unter dem Kreis
