...
- Erste Ansicht: `/tablet/temperatures` (`pages/tablet-temperatures/`). Alle Sensoren gleichzeitig ohne Scrollen (2 Spalten, ab 6 Sensoren 3; `grid-auto-rows: 1fr` in `100dvh`), Temperatur **und** Luftfeuchte in einem Chart mit zwei Y-Achsen statt eines Umschalters — auf einer Wandanzeige ist Antippen lästig. Selbst-Refresh alle 5 Minuten; ein fehlgeschlagener Refresh behält die letzten Werte, nur der Erstabruf meldet einen Fehler
