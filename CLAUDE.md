...
- Frontend: Leaflet + OpenStreetMap-Kacheln, Seite `pages/pets/` unter Route `/pets` („Hundetracker", Navi unter „Smart Home"). Die Leaflet-Standard-Marker-Icons werden lokal ausgeliefert (`angular.json`-Assets-Glob → `assets/leaflet`), **nie von einem CDN** — das Dashboard muss ohne Internet funktionieren
