...
- **Switch Confirmation**: `confirm_required` flag on `entity_states`, applied in three places — the dashboard switch tile, the device page, and the helpers page (`/custom-entities`, `INPUT_BOOLEAN`)
  ...
  - All three surfaces re-resolve the entity by id from the current list right before confirming (for the dashboard's "Alle Schalter" dialog that list, `allSwitches`, is loaded once in `openSwitchDialog` and not touched by the 30 s refresh, so it can be minutes stale), and only proceed if it is still on — a background refresh while the dialog is open must never end up switching the thing back ON via the "Ausschalten" button (`DashboardComponent.confirmToggle`, `SmartDeviceListComponent.confirmTurnOff`, `CustomEntitiesComponent.confirmTurnOff`). `DashboardComponent.confirmToggle` additionally falls back to the dialog's held reference if the switch turns up in neither list (emptied by a failed refresh, or pushed out of the top-8 tile limit) — not found is not evidence it's off, since the dialog only ever opens for a switch that was on
...
- **Spaziergänge-Dialog:** Klick auf die Hund-Kachel im Dashboard-Footer öffnet einen Dialog mit den Spaziergängen der letzten 7 Tage. Es gibt keinen Walks-Endpunkt in der Tractive-API (das App-Feature ist nicht reverse-engineert) — `GET /v1/tractive/pets/{trackerId}/walks?days` leitet sie stattdessen aus Positionspunkten ab: **Einschalt-Indikator dieses Haushalts** (`TractiveWalkDetector`, Entscheidung 2026-07-28): Der Tracker ist zu Hause aus und wird nur für die Runde eingeschaltet — ein Spaziergang ist deshalb ein zusammenhängender Block von Positionsberichten zwischen zwei Funkpausen > 30 min (Blockränder ≈ Ein-/Ausschalten, deckt sich mit den App-Gassirunden), nicht erst die Zeit außerhalb des Home-Radius. Blöcke < 5 min werden verworfen; ein Block ohne einen einzigen Punkt außerhalb des Home-Radius zählt nicht (sonst würde die Ladeschale zum Spaziergang). **Kehrseite:** bleibt der Tracker unterwegs dauerhaft an, wird der ganze Zeitraum ein einziger langer „Spaziergang" — ändert sich die Ein/Aus-Gewohnheit des Haushalts, muss der Detector zurück auf die Home-Radius-Logik. Ohne konfiguriertes Zuhause 400 mit klarer Meldung
- **Die Punkte kommen aus der eigenen Historie, nicht mehr aus der Cloud** (Umbau 2026-08-23): `TractivePositionRecorder` schreibt im minütlichen Poll-Zyklus jede Position nach `tractive_position` — das kostet **keinen** zusätzlichen Cloud-Aufruf, der Poller hat sie ohnehin in der Hand. `TractiveWalkService` liest daraus und gibt die Zeilen als `TractivePositionDto` an den unveränderten Detector
- **Anlass:** die Cloud liefert beim Basic-Abo nur ~24 h Historie, und der frühere `dayCache` war ein reiner **Abruf**-Cache — nur im Speicher, weg beim Neustart, und er konnte nur Tage enthalten, die die Cloud schon einmal geliefert hatte. In der Tablet-Ansicht war deshalb praktisch nur die aktuelle Runde sichtbar, egal welcher Zeitraum gewählt war
- **Gespeichert wird nur ein NEUER Bericht** (`existsByTrackerIdAndPositionTime` plus Eindeutigkeitsschlüssel auf `(tracker_id, position_time)`): bei ausgeschaltetem Tracker liefert die API weiter die **letzte bekannte** Position mit unverändertem Zeitstempel. Würde die jede Minute gespeichert, entstünde ein künstlich lückenloser Strom — und der Detector trennt die Runden **gerade an den Funkpausen**. Das Ergebnis wäre ein einziger, nie endender „Spaziergang"
- `TractivePositionRecorder` **wirft nie** (Muster `PowerHistoryRecorder`): derselbe Poll versorgt Entitäten, Dashboard-Kachel und Zu-Hause-Sensor. Er läuft **vor** `updates.forEach(entityStateService::reportState)` — diese Zeile steht in keinem `try/catch`, ein danach platzierter Recorder käme bei einem Fehler dort nie zum Zug. Ein Test hält die Reihenfolge fest
- **Damit entfallen:** Tages-Cache, Rate-Limit-Behandlung, 24-h-Häppchen und die **Anmeldeprüfung** im Lesepfad (`TractiveWalkService` schrumpfte von 166 auf 80 Zeilen). Gespeicherte Runden bleiben sichtbar, auch wenn das Tractive-Token abgelaufen ist — was mangels Refresh-Token regelmäßig passiert. `MAX_DAYS` ist von 14 auf **365** gestiegen und nur noch Eingabevalidierung
...
- `TractiveApiClient.getPositionHistory` wurde dabei entfernt (nur noch von seinem eigenen Test aufgerufen). Die daran erarbeiteten Erkenntnisse bleiben hier erhalten: der Abruf brauchte zwingend **Tages-Häppchen** (die Cloud lehnt größere Fenster mit Code 7500 HISTORY ab, real beobachtet bei 7 Tagen) und die Positions-Ressource ist **rate-limitiert** (429, Code 4006, real beobachtet). Wer je aus der Cloud nachfüllen will, holt die Methode aus der Git-Historie zurück
...
### Tablet-Ansichten (Unterseiten des Wandtablets)
- **Im Tablet-Modus (`ViewModeService.isTabletView()`) blendet `app.component.html` den Header komplett aus** — es gibt dort keine Navigation. Deshalb die Leiste `lumina__viewbar` unter dem Dashboard-Footer, und deshalb braucht **jede** verlinkte Tablet-Seite einen eigenen Zurück-Knopf; ohne ihn ist das Tablet in der Unterseite gefangen
- Die Einträge stehen als Array `tabletViews` in `dashboard.component.ts`, ein weiterer kostet eine Zeile. Markup und Styles liegen **direkt im Dashboard** — die `lumina`-Styles sind in `dashboard.component.scss` gekapselt und griffen in einer Kind-Komponente lautlos nicht (siehe Tractive-/Zigbee-Kacheln)
- Jede Tablet-Unterseite steckt in `components/tablet-shell/` (`<app-tablet-shell heading="…">`): oben dieselbe Uhr-/Wetterzeile wie im Dashboard, unten dieselbe Ansichtsleiste (zusätzlich „Dashboard" als Rückweg, aktive Ansicht hervorgehoben), dazwischen der projizierte Inhalt. **Geteilt sind die Daten, nicht das CSS** — `TABLET_VIEWS` (`shared/tablet-views.ts`) und `weatherMaterialSymbol` (`shared/weather-icon.util.ts`) haben genau eine Definition, die Kopf-/Leistenstile stehen wegen der `lumina`-Kapselung zwangsläufig zweimal. Ändert sich die Optik der Kopfzeile, sind beide Dateien fällig
- Die Shell zeigt **keine** Außenfühler-Chips wie das Dashboard: die hängen dort am Sensor-Detaildialog, der zum Dashboard gehört
- Erste Ansicht: `/tablet/temperatures` (`pages/tablet-temperatures/`). Alle Sensoren gleichzeitig ohne Scrollen (2 Spalten, ab 6 Sensoren 3, `grid-auto-rows: 1fr`). Selbst-Refresh alle 5 Minuten; ein fehlgeschlagener Refresh behält die letzten Werte, nur der Erstabruf meldet einen Fehler
- **Die Höhe kommt über eine durchgehende Flex-Kette** von `.app-layout` (100vh) über `:host` der Seite, die Shell und das Raster bis zum Chart-Element; ECharts liest die Höhe per `ResizeObserver` (`autoResize`). Fehlt `flex: 1` oder `min-height: 0` an **einer** Stelle, fällt alles darunter auf Inhaltshöhe zurück und die Graphen schrumpfen auf fast null — genau so passiert, weil das Host-Element der Seite die Regel zunächst nicht hatte. Ein Test in `tablet-temperatures.component.spec.ts` misst die Chart-Höhe bei 600 px und 900 px und hält die Kette fest
- Kopfzeile rechts: Zeitraum (24 h / 7 Tage / 30 Tage, Default 7 Tage) und Messgrößen (Temperatur / Luftfeuchte, **Default nur Temperatur**), beide über den Slot `[shellActions]` der Shell projiziert. Messgrößen sind unabhängig schaltbar; die **letzte aktive lässt sich nicht abwählen**, sonst stünde die Wand voll leerer Diagramme. Umschalten baut die Charts aus den gehaltenen Rohserien neu — kein neuer Abruf
- Die Kacheln tragen den **Kurznamen** des Sensors (`shortenSensorName` in `shared/temperature-comfort.util.ts`, einzige Definition — dieselbe Funktion beschriftet die Außenfühler-Chips im Dashboard): das führende „Temperatur"/„Temp." entfällt
- Hat ein Sensor zur gewählten Auswahl keine Werte (Außenfühler ohne Feuchte bei „nur Luftfeuchte"), zeigt die Kachel einen Hinweis statt eines leeren Diagramms
- Die Website-Seite `/temperatures` bleibt unverändert. Geteilt wird nur `TemperatureService`; die Chart-Optionen gibt es **zweimal**, weil sie unterschiedliche Ziele haben — eine Änderung an der Darstellung muss bewusst entschieden werden, sie wirkt nicht automatisch auf beiden Seiten
- `GET /v1/temperatures` ist über die generische `GET /v1/**`-Regel KIOSK-lesbar; die Route braucht keine eigene Security-Zeile
...
- **Anders als bei den Temperaturen gibt es keinen Messgrößen-Umschalter:** jede Messgröße jedes Sensors bekommt ihre **eigene Kachel**, alle gleichzeitig sichtbar (Draußen: PM2.5, PM10; je Amazon-Monitor: IAQ, PM2.5, VOC, CO — bei einem Monitor also 6 Kacheln, bei zweien 10). Mehrere Größen in einem Diagramm wären ohnehin nicht möglich, sie haben vier verschiedene Einheiten (µg/m³, Score 0–100, ppb, ppm). Reihenfolge und Anzeige-Eigenschaften: `AIR_QUALITY_METRICS` in `models/air-quality-series.model.ts`; das Raster nimmt bis 2 Kacheln 2 Spalten, bis 6 drei, darüber vier
- Eine Kachel entsteht nur für eine Messgröße, die der Sensor **tatsächlich liefert** — es gibt weder leere Diagramme noch „keine Werte"-Hinweise
- In der Kachelüberschrift steht die Messgröße groß, der Sensorname klein daneben und rechts der jüngste Wert
- **Farbe trägt hier Bedeutung, nicht Identität:** die Linien haben keine eigene Farbe je Messgröße, sondern werden nach Grenzwerten eingefärbt (ECharts-`visualMap`, stückweise über den Messwert) — grün, gelb, orange, rot, dunkelrot. Derselbe Wert färbt auch die Zahl in der Überschrift, weil beide `airQualityLevel` fragen. Einzige Definition: `shared/air-quality-thresholds.util.ts`
- **Herkunft der Schwellen** (steht im Kopf der Util): PM2.5/PM10 aus den Bändern des europäischen Luftqualitätsindex der EEA; CO aus den WHO-Innenraumrichtwerten (8-h ≈ 9 ppm, 1-h 35 ppm). Beim **IAQ** bleibt `iaqLevel` (65/35) die einzige Schwellenquelle, statt eine zweite Zahlenreihe zu erfinden — die Skala ist dort umgekehrt (hoch = gut), die Färbung ebenfalls. **VOC hat keinen amtlichen Grenzwert**: die Bänder (65/220/660/2200 ppb) sind die in der Sensorik verbreiteten TVOC-Stufen, und Amazon dokumentiert weder Messverfahren noch Bezugsgas — ein Anhaltspunkt, keine Gesundheitsaussage
...
- **Der Höhenketten-Test braucht einen eigenen Container, nicht `host.parentElement`:** das Fixture hängt in Karma direkt im `<body>`, zusammen mit Karmas eigenen Elementen und den Wurzelknoten schon gelaufener Suiten. Macht der Test den Body zur Flex-Spalte mit fester Höhe, teilen sich all diese Geschwister die 600 px und der Graph wird je nach Suite-Reihenfolge winzig — allein lief die Datei grün, im Gesamtlauf fiel sporadisch mal die Temperatur-, mal die Luftqualitätsansicht durch. Beide Tests hängen den Host deshalb in ein selbst erzeugtes `div` um
- Dritte Ansicht: `/tablet/toni` (`pages/tablet-toni/`). 2×2-Raster: Futtervorrat, Spaziergänge, Tracker-Status, Karte. **Rein anzeigend** — auf dem KIOSK-Wandtablet wären Futter-Buchungen ohnehin MEMBER und lieferten 403; alle drei gelesenen Endpunkte sind über die generische `GET /v1/**`-Regel KIOSK-lesbar, die Seite braucht **keine** eigene Zeile in `SecurityConfig`
- Die drei Quellen (`PetFoodService.getStatus`, `TractiveService.getPets`, `TractiveService.getWalks`) laufen **unabhängig**: fällt Tractive aus, steht der Futtervorrat trotzdem da. Die Spaziergänge hängen an der `trackerId` aus `getPets` und werden ohne Tracker gar nicht erst angefragt. Wie bei den anderen Tablet-Ansichten meldet **nur der Erstabruf** einen Fehler; ein fehlgeschlagener Hintergrund-Refresh behält die letzten Werte stumm
...
- Der Zeitraum der Spaziergänge ist 7 / 14 / 30 Tage. Seit dem Umbau auf die eigene Positionshistorie (2026-08-23) kostet ein Zeitraumwechsel **keinen** Cloud-Aufruf mehr und kann nicht am Rate-Limit scheitern; `MAX_DAYS` (365) ist nur noch Eingabevalidierung. **Die Kachel füllt sich erst ab dem Deploy** — ältere Runden gibt die Cloud beim Basic-Abo nicht mehr her
- Tage ohne Runde sind eine **leere Säule**, kein fehlender Eintrag — eine Lücke sähe aus wie ein Datenloch
- **Der Zeitraumumschalter sitzt in der Kachel, nicht im `[shellActions]`-Slot** wie bei Temperatur und Luftqualität: dort gilt er für die ganze Seite, hier nur für diese eine Kachel. `setWalkDays` baut das Diagramm deshalb **selbst** neu, bevor nachgeladen wird — die Achse soll die neue Länge sofort zeigen. Ein Test mit `of(...)` kann das nicht belegen (der synchrone Stub verdeckt den Aufruf), deshalb steht dafür ein eigener Test mit einem `Subject`
- Der Kartencontainer steht **immer** im DOM, der Hinweis bei fehlender Position liegt als Overlay darüber. Läge er hinter einem `@if`, hinge der Kartenaufbau an der Reihenfolge von Datenankunft und Change Detection — **genau daran krankt `/pets`:** dort läuft der erste `renderMap`-Aufruf ins Leere und die Karte erscheint erst beim nächsten Poll nach 60 s. Die Toni-Ansicht nutzt stattdessen `ViewChild` + `ngAfterViewInit`
- Die OSM-**Kartenkacheln** kommen aus dem Internet: ohne Verbindung bleibt diese eine Kachel grau, die anderen drei laufen weiter. Die **Marker-Icons** kommen lokal aus `assets/leaflet` (`shared/leaflet-icons.util.ts`, geteilt mit `/pets`) — nie von einem CDN
- **Der Höhenketten-Test dieser Ansicht misst bei 900 und 1200 px, nicht bei 600 und 900 wie die beiden anderen.** Die Spaziergangs-Kachel trägt über und unter dem Graphen festen Inhalt (Zeitraumknöpfe, Klartextliste); bei 600 px blieben dem Graphen strukturell ~24 px, ohne dass an der Kette etwas kaputt wäre. Ein gebrochenes Glied fängt der Test trotzdem — dann wächst der Graph gar nicht mehr mit dem Bildschirm
...
- **Balken derselben Periode werden zusammengefasst** (`aggregateByPeriod`, eine Methode für beide Auflösungen): zwei Ablesungen derselben ISO-Woche — die Freitagsablesung plus eine Korrektur am Samstag — trügen sonst dasselbe Label, und auf der **Kategorienachse** des Diagramms landeten beide Werte auf derselben Position; ein Balken verschwände lautlos. Der Wochenschlüssel enthält deshalb ISO-**Jahr und** -Woche: bei `WEEKS_52` umfasst das Fenster zwei Kalenderjahre
- **Eine Ablesewoche über dem Monatswechsel zählt vollständig in den Monat ihres Ablesedatums.** Bewusst gegen tagesgenaues Aufteilen: der Balken entspricht so weiterhin echten Ablesungen. Preis: an Monatsgrenzen leicht unscharf. Ein Balken gilt als `estimated`, sobald **mindestens eine** beitragende Ablesung geschätzt war — sonst verschwände eine geschätzte Woche in einem sonst echten Monat spurlos
...
- **Ein Zeitraumwechsel bestellt den laufenden Abruf ab** (`pendingRequest`), bevor der nächste startet: sonst könnte bei schnellem Umschalten eine ältere, langsamere Antwort nach einer neueren eintreffen und die schon richtigen Kacheln mit Zahlen zum falschen Zeitraum überschreiben. Die Schwesteransichten `tablet-temperatures` und `tablet-air-quality` haben diese Absicherung **nicht** — wer dort dasselbe Verhalten will, muss es nachziehen
- Der Vorperiodenvergleich im Kachelkopf ist bewusst **nicht eingefärbt**: ob mehr Verbrauch schlecht ist, hängt an Jahreszeit und Anlass. Er entfällt wortlos bei weniger als zwei Punkten oder wenn die Vorperiode 0 war (Division durch null), und er sagt „ggü. **letztem Wert**" statt „ggü. Vormonat", wenn zwischen den beiden Balken Perioden fehlen — der Endpunkt lässt Perioden ohne Ablesung ganz weg, die letzten beiden Balken müssen also nicht benachbart sein
...
- Die Monatsbeschriftung kommt aus `DateTimeFormatter.ofPattern("MMM yy", Locale.GERMAN)` und ist **nicht durchgehend dreistellig**: Mai bis Juli kürzen sich laut CLDR gar nicht („Juli 26"), die übrigen tragen einen Punkt („Dez. 26")
...
### Netzwerk-Monitoring
- Modul `backend/src/main/java/com/household/manager/network/` mit drei **unabhängigen** Messpfaden (Fehler in einem stört die anderen nie; alle Scheduled-Methoden werfen nie): Internet-Status/Latenz minütlich (`NetworkConnectivityPollingService`), Cloudflare-Speedtest stündlich (`NetworkSpeedtestService`), LAN-Geräte-Checks minütlich (`NetworkDevicePollingService`). Spec: `docs/superpowers/specs/2026-08-24-netzwerk-monitoring-design.md`
- **Kein ICMP-Ping, nirgends:** im Docker-Bridge-Container fällt Javas `InetAddress.isReachable` ohne Raw-Socket-Rechte still auf TCP-Port 7 zurück und meldete dauerhaft „offline". Stattdessen HTTP-Checks gegen zwei unabhängige Ziele (`https://1.1.1.1/cdn-cgi/trace` per IP — funktioniert auch bei kaputtem DNS — und `https://www.gstatic.com/generate_204`); online = mindestens ein Ziel antwortet, Latenz = Minimum. Gateway getrennt per TCP-Connect 80→443 (`network.gateway-ip`, Default 192.168.1.1) — damit ist „Router weg" von „Leitung weg" unterscheidbar. Geräte-Checks ebenfalls TCP: konfigurierter Port oder Fallback-Reihe 80, 443, 22, 1883, 8080, 8443
- **Speedtest misst gegen `speed.cloudflare.com`** (`__down`/`__up`), reines Java, **HTTP/1.1 erzwungen** (HTTP/2-Flow-Control würde die Messung verfälschen), Zeitbudget `network.speedtest.budget-seconds` (10 s) je Richtung, Messung ab dem ersten Byte. Ein fehlgeschlagener Test wird als Fehlschlag **gespeichert** (sichtbare Lücke mit Grund); scheitert nur eine Richtung, gilt `success = mindestens eine Richtung lieferte` und nur die erfolgreiche meldet ihre Entität. Kein Test, solange der letzte Connectivity-Sample offline ist. Der manuelle Trigger (`POST /v1/network/speedtest`, 200 mit `SpeedtestSummary`) reserviert seinen 60-s-Cooldown **per compareAndSet VOR der Messung** — sonst messen Doppelklick/zwei Tabs doppelt; ein späterer Messfehler rollt den Cooldown bewusst nicht zurück. Cooldown-Verstoß ⇒ 429 (`TooManyRequestsException`, eigener Handler), offline ⇒ 400
- Historie: `network_connectivity_sample` (minütlich; **Lese**-Downsampling über den bestehenden `SeriesDownsampler`, bewusste Abweichung von der Spec — bei 1440 Zeilen/Tag wäre Schreib-Kompaktierung unnötige Komplexität) und `network_speedtest_result`. Retention löscht nachts 03:20 (bewusst nicht zur vollen Stunde — dort läuft der Speedtest): Samples nach 30 Tagen, Speedtests nach 365. **Die `deleteBy…`-Methoden sind `@Transactional @Modifying @Query`-Bulk-Deletes** (Muster `WasteCollectionEventRepository`) — abgeleitete Delete-Methoden bringen KEINE eigene Transaktion mit und würfen gefangen `TransactionRequiredException`: die Retention wäre dauerhaft still wirkungslos (real im Review gefunden, Mockito-Tests sind dafür strukturell blind)
- Entitäten (`EntitySource.NETWORK`): `binary_sensor.network_internet` (`deviceClass: connectivity`; Attribute `gatewayReachable`, `latencyMs` — der Schlüssel fehlt bei offline, statt `null` zu tragen), `sensor.network_latency_ms` (nur bei online gemeldet), `sensor.network_download_mbps`/`_upload_mbps` (nur nach erfolgreicher Messung der jeweiligen Richtung). Diese Entitäten werden **nie `unavailable`** — die Messung ist lokal, „Internet weg" ist der Zustand `off`, kein Quellenausfall. **Ein Telegram-Flow auf `off` stellt seine Nachricht naturgemäß erst nach Rückkehr des Internets zu** (Telegram ist beim Ausfall selbst unerreichbar) — kein Fehler, sondern Physik; die `on`-Flanke eignet sich für „Internet war weg von–bis". Kein Trigger auf `value: "unavailable"` (tote-Trigger-Falle)
- LAN-Gerätestatus lebt **nur im Speicher** (`NetworkDeviceStatusMonitor`, Muster `ZigbeeStreamMonitor`): überlebt Neustarts nicht, keine Historie je Gerät (bewusste v1-Grenze). `lastSeenAt` friert beim Ausfall auf dem letzten Erreichbar-Zeitpunkt ein, `lastCheckedAt` läuft weiter; nie erreichbar ⇒ `null` („–" im UI). Die Geräteliste ist DB-gepflegt (`network_device`, Admin-Seite „Netzwerk-Geräte", Route `admin/network-devices`, Muster Kalender-Kategorien inkl. Voll-PUT beim Aktiv-Toggle — ein Teil-PUT würde `active` serverseitig als „aktiv" lesen); Audit `network.device.create/update/delete`
- API `/api/v1/network`: `GET /status` (alles für die Tablet-Seite in einem Abruf; **ohne ein einziges Sample** antwortet er `online=false` mit `lastCheckedAt=null` — das Frontend zeigt dafür neutral „Noch keine Messung" statt rot), `GET /history?range=DAY|WEEK|MONTH`, `POST /speedtest`, Geräte-CRUD. **Alle Zeitstempel im Antwortbaum sind LocalDateTime (lokale Haushaltszeit)** — `lastSeenAt` wird dafür aus dem `Instant` des Monitors über `clock.getZone()` konvertiert; ein gemischtes Instant/LocalDateTime-Format im selben Baum war ein realer Review-Fund
- Security: Lesen über die generische `GET /v1/**`-Regel KIOSK; `POST /v1/network/speedtest` steht in der KIOSK-POST-Whitelist (zieht nur Daten — sonst wäre der Knopf auf dem Wandtablet tot, Muster Tractive-Refresh); Geräte-Schreibzugriffe sind drei **methodenspezifische** ADMIN-Matcher vor den generischen Regeln (Muster Kalender-Kategorien), jede der drei Zeilen hat ihren eigenen MEMBER-Verbotstest in `SecurityRulesTest`
- Tablet-Ansicht `/tablet/network` (`pages/tablet-network/`): 2×2 — Status (mit „Jetzt testen"-Knopf, 429-Meldung inline), Speed-Verlauf (Punkte sichtbar, ~24/Tag), Latenz-Verlauf, Geräteliste. Selbst-Refresh 60 s, nur der Erstabruf meldet Fehler; `pendingRequest`-Abbruch beim Zeitraumwechsel (wie Verbrauch). **Chart-Optionen sind Felder, keine Methodenaufrufe im Template** — `[options]="methode()"` würde bei jeder Change-Detection ein neues Objekt liefern und ngx-echarts zu einem kompletten Redraw (`notMerge: true`) zwingen. Die Latenzlinie bricht bei Lücken **über** dem Dreifachen der Bucket-Länge (DAY 5 min / WEEK 30 min / MONTH 2 h, gespiegelt aus `SeriesRange`; exakt das Dreifache ist noch normal) — `insertGaps` in `pages/tablet-network/network-view.util.ts`, `connectNulls: false`
- **Der fünfte Eintrag in `TABLET_VIEWS` hat die Ansichtsleiste umbrechen lassen** und damit den Höhenketten-Test der Luftqualitätsansicht deterministisch gebrochen (die Leiste stahl dem Raster Höhe — auch real ein Tablet-Problem, kein Testartefakt): `.lumina__viewbar` ist seitdem `flex-wrap: nowrap` + `overflow-x: auto` mit kompakteren Knöpfen, **in beiden Stil-Kopien** (Shell UND Dashboard, lumina-Kapselung). Wer künftig Einträge ergänzt: die Leiste scrollt seitwärts statt umzubrechen
- Bewusste v1-Grenzen: keine Router-Interna (TP-Link AX6000 hat kein offenes API — „Router" heißt Gateway-Erreichbarkeit), keine Historie je LAN-Gerät, Speedtest misst die Anbindung des **Servers** (LAN-verkabelt), nicht die WLAN-Erfahrung eines Endgeräts; stündliche Tests kosten Traffic (grob 5–10 GB/Tag, akzeptiert). Keine Website-Seite — nur Tablet; Service/API sind seitenneutral
...
- **Manueller Abruf** `POST /v1/presence/refresh` (Knopf „Jetzt prüfen" auf der Admin-Seite): läuft **synchron** über denselben Zyklus wie der Poller — ein zweiter Probe-Pfad könnte vom geplanten abweichen — und liefert den frischen Status in derselben Antwort zurück. Er kostet `aktive Geräte × 3 Ports × Timeout` auf dem Anfrage-Thread (Default rund 6 s je stillem Gerät), deshalb **ADMIN und bewusst NICHT in der KIOSK-POST-Whitelist** (anders als Tractive-Refresh und Speedtest, die das Wandtablet braucht), und deshalb weist ein `AtomicBoolean` einen zweiten gleichzeitigen Abruf mit 429 ab — Freigabe im `finally`, sonst bliebe der Knopf nach einem Fehler dauerhaft blockiert. Überschneidung mit dem *geplanten* Lauf ist ungefährlich und bewusst ungeschützt (`PresenceMonitor.update` ist über `compute` atomar). Anders als `poll()` **verschluckt er Fehler nicht**: wer den Knopf drückt, soll erfahren, dass die Geräteliste nicht geladen werden konnte
- Frontend: Admin-Seite „Anwesenheit" (`pages/admin-presence/`, Route `admin/presence`) mit Karenzzeit, Geräteliste (Personen-Dropdown aus `GET /v1/users`) und der Spalte „Zuletzt gesehen" aus `GET /status`; schlägt dieser Abruf fehl, bleibt die Spalte leer, ohne die Pflege zu blockieren. Anzeige seit 2026-08-26 in der Dashboard-**Kopfzeile** (`lumina__clock`, lumina-Kapselung, nicht mehr im Footer): ein Kreis mit Status-Icon (`presenceIcon`, seit der Nachbesserung 2026-08-26 statt einer Initiale) und sichtbarem Namen je Person, Refresh 30 s, ein fehlgeschlagener Refresh behält den letzten Stand. `presenceRingClass`/`presenceLabel`/`presenceIcon` zählen alle vier Zustände einzeln auf und haben **kein `default`** — mit `noImplicitReturns` wird eine künftige fünfte Ausprägung von `PresencePersonState` zum Compilerfehler statt still auf dem falschen Ring/Symbol zu landen (Gegenstück zu `PresenceEvaluator.entityState`). Der Ring selbst hat weiterhin nur **drei** visuelle Zustände: `unavailable` und `unknown` teilen sich denselben neutralen Ring (grau) — eine bewusste, für dieses Wandtablet getroffene Entscheidung, dass beide NIE wie `off` (rot) aussehen dürfen. Anders als beim ersten Umbau tragen sie dabei aber **unterschiedliche Icons** (`signal_disconnected` vs. `help`) und sind damit auch ohne den auf dem berührungslos laufenden KIOSK-Wandtablet unerreichbaren `title`/`aria-label`-Tooltip auf einen Blick unterscheidbar; `off` bekommt zusätzlich reduzierte Deckkraft als farbunabhängige zweite Ebene (Rot-Grün-Sehschwäche) plus eine dritte, ebenfalls farbunabhängige Ebene durch die Icon-Form selbst, und der Name steht sichtbar unter dem Kreis
- **Zwei WLAN-Bänder = zwei Geräte-Einträge je Handy** (real gefunden 2026-08-27, kostete eine lange Fehlersuche): iOS vergibt seine private WLAN-Adresse **pro SSID**. Heißen 2,4-GHz- und 5-GHz-Netz unterschiedlich, hat dasselbe iPhone **zwei MACs und damit zwei IPs**, und es pendelt im Betrieb zwischen ihnen. Ist nur eine davon erfasst, antwortet das Handy scheinbar in unregelmäßigen Abständen gar nicht — beobachtet wurden Lücken von 13+ Minuten, die wie ein Schlafmodus aussehen, aber keiner sind. Abhilfe ohne Code: **beide Adressen als je einen Geräte-Eintrag derselben Person** anlegen (anwesend ist, wessen *irgendein* Gerät antwortet). Dauerhaft sauber wird es erst mit „Private WLAN-Adresse: **Fest**" in **beiden** Netzen plus DHCP-Reservierung für **beide** MACs — oder indem beide Bänder dieselbe SSID bekommen, dann bleibt es bei einer Adresse
- **Diagnose-Rezept, wenn eine Person fälschlich „abwesend" ist:** `GET /v1/presence/status` liefert `lastSeenAt` je Gerät — steht es fest, während die Person da ist, ist die IP tot. Das Handy im Netz wiederfinden: Ping-Sweep über das Subnetz, dann die Nachbartabelle nach Einträgen mit **lokal verwalteter MAC** durchsuchen (zweites Bit des ersten Bytes gesetzt, z. B. `4a:…`, `da:…` — das kennzeichnet iOS' private Adressen) und die Kandidaten auf **TCP 62078** (iOS lockdownd) prüfen; nur iOS-Geräte haben ihn. Achtung: der Port ist nicht durchgehend offen, Port 80 antwortet dagegen zuverlässig mit einer **Ablehnung**, was die Probe korrekt als „lebt" wertet
