...
- [kasa-bulb-support.md](kasa-bulb-support.md) - legacy Kasa (KL110); gemessen: ignore_default+on_off Pflicht (sonst stiller No-op), err_code:0 beweist nichts; Dimmer-Faehigkeits-Bug (DeviceType allein reicht nicht); LightState nach smartdevice/ verschoben
- [value-annotation-single-constructor.md](value-annotation-single-constructor.md) - @Value + Konstruktor-Injektion: Feld-Injektion (RequiredArgsConstructor) vs. ein manueller Konstruktor mit allen Feldern; zweiter noetig wenn ein Unit-Test den @Value-Wert kontrollieren muss
- [derived-delete-not-transactional.md](derived-delete-not-transactional.md) - abgeleitete deleteBy...-Methoden sind NICHT automatisch transaktional; Retention-Deletes als @Modifying-Bulk-Delete (Muster WasteCollectionEventRepository); Mockito-Tests sehen den Bug nicht
- [cloudflare-speedtest-download-limit.md](cloudflare-speedtest-download-limit.md) - Cloudflare __down hat unbekanntes Groessenlimit (<100 MB), darueber 403 mit 1-Byte-Body statt Fehler; Statuscode immer pruefen; Real-Net-Verifikation als geloeschte main()-Klasse
- [presence-manual-refresh.md](presence-manual-refresh.md) - POST /v1/presence/refresh (2026-08-26); geteiltes runCycle(), AtomicBoolean-Guard, ADMIN-only, TooManyRequestsException wiederverwendet
- [mockito-restub-after-thenthrow.md](mockito-restub-after-thenthrow.md) - when(mock.x()).thenReturn nach thenThrow loest die alte Exception erneut aus; doReturn/doThrow fuer den Restub verwenden
- [presence-wifi-review-fixes.md](presence-wifi-review-fixes.md) - Review-Runde 2026-08-26: Reuse-Entscheidung war stale (2. Aufrufer), Javadoc-Scoping-Muster, Thread.setUncaughtExceptionHandler fuer Nebenlaeufigkeitstests
- [blink-security-rules.md](blink-security-rules.md) - Blink-Snapshot in KIOSK-Whitelist, Arm/Disarm bleibt MEMBER; beide Mutationsproben bestaetigt (fa77c35)
